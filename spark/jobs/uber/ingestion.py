...
# --- CẤU HÌNH ---
MINIO_CONF = {
    "endpoint": "localhost:9000",      # Đổi thành minio:9000 nếu chạy trong Docker
    "access_key": "admin",
    "secret_key": "password",
    "secure": False
}
...
# Tự động tìm file mẫu ncr_ride_bookings.csv
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# Giả sử bạn để file csv trong folder 'data' cùng cấp với script
SOURCE_FILE = os.path.join(SCRIPT_DIR, "data", "ncr_ride_bookings.csv") 
...
class UberDataGenerator:
    ...
    def _learn_from_sample(self):
        """Đọc file CSV mẫu để lấy danh sách địa điểm và loại xe thực tế"""
        ...
        if not self.locations:
            self.locations = ['District 1', 'District 7', 'Airport', 'Thao Dien', 'Binh Thanh']
        if not self.vehicle_types:
            self.vehicle_types = ['Auto', 'Bike', 'Go Sedan', 'UberXL', 'Premier Sedan']
    ...
    def generate_day(self, target_date_str, num_rows):
        # Hàm này nhận num_rows từ bên ngoài truyền vào
...
def upload_to_minio(df, date_str):
    """Upload DataFrame lên MinIO với tên file tùy chỉnh theo ngày"""
    client = Minio(
...
# --- MAIN EXECUTION (CHẾ ĐỘ BACKFILL RANDOM) ---
if __name__ == "__main__":
    generator = UberDataGenerator(SOURCE_FILE)
    
    # Cấu hình ngày chạy (Chạy thử 3 ngày để xem sự biến động)
    ...
    while current_date <= end_date:
        ...
        # [THAY ĐỔI]: Random số lượng dòng trong khoảng 400 - 450
        rows_today = random.randint(400, 450)
        print(f"\n>>> Day: {date_str} | Volume: {rows_today}")
        
        # 1. Sinh dữ liệu với số lượng ngẫu nhiên vừa tạo
        df_daily = generator.generate_day(date_str, num_rows=rows_today)
        
        # 2. Upload
        upload_to_minio(df_daily, date_str)
